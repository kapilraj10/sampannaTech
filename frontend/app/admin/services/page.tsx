...
const iconOptions = [
  'Globe', 'Code', 'Smartphone', 'LayoutDashboard', 'PenTool', 'Lightbulb',
  'Network', 'LifeBuoy', 'ShoppingCart', 'Cloud', 'Database', 'Shield',
  'BarChart3', 'Users', 'Mail', 'Settings', 'Boxes', 'LineChart',
];
...
export default function AdminServicesPage() {
  const { items, loading, error, load, create, update, remove } = useAdminCrud<ServiceItem>(
    'services',
    '',
    ...
    <>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center">
          <p className="text-sm text-slate-500">No services yet. Add your first service.</p>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className="hidden px-5 py-3 font-semibold sm:table-cell">Slug</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => (
                <tr key={item._id} className="hover:bg-slate-50">
                  ...
                  <td className="hidden px-5 py-3.5 text-slate-500 sm:table-cell">{item.slug}</td>
                  <td className="px-5 py-3.5 text-slate-600">{item.order}</td>
